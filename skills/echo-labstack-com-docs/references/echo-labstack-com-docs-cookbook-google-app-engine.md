...
We take advantage of the go [build constraints or tags](https://golang.org/pkg/go/build/) to change
...
initialized but *before* the `main()` function is executed. These allow additional handlers to attach
...
AppEngine Classic and Managed VM(s)[​](#appengine-classic-and-managed-vms "Direct link to AppEngine Classic and Managed VM(s)")
-------------------------------------------------------------------------------------------------------------------------------
...
- url: /.*  
  script: _go_app
```
### Router configuration[​](#router-configuration "Direct link to Router configuration")
We'll now use the [build constraints](https://golang.org/pkg/go/build/) again like we did when creating
