...
Testing
=======
Testing Handler[​](#testing-handler "Direct link to Testing Handler")
---------------------------------------------------------------------
`GET` `/users/:id`
Handler below retrieves user by id from the database. If user is not found it returns
`404` error with a message.
### CreateUser[​](#createuser "Direct link to CreateUser")
`POST` `/users`
...
```
### Using Form Payload[​](#using-form-payload "Direct link to Using Form Payload")
```
// import "net/url"  
f := make(url.Values)  
f.Set("name", "Jon Snow")  
...
```
### Setting Path Params[​](#setting-path-params "Direct link to Setting Path Params")
```
c.SetParamNames("id", "email")  
c.SetParamValues("1", "jon@labstack.com")
```
### Setting Query Params[​](#setting-query-params "Direct link to Setting Query Params")
```
// import "net/url"  
q := make(url.Values)  
...
```
Testing Middleware[​](#testing-middleware "Direct link to Testing Middleware")
------------------------------------------------------------------------------
*TBD*
